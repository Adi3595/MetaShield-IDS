"""
MetaShield — Distributed Denial of Service (DDoS) Test Script
==============================================================
Simulates DDoS-SYN-Flood and DDoS-UDP-Flood attack subtypes.
...
def generate_ddos_features(dim, rng, subtype="SYN-Flood"):
    """
    Generate synthetic features that mimic DDoS attack patterns.
    DDoS traffic: multiple sources, extreme volume, spoofed IPs,
    different protocols depending on flood type.
    """
    flow = np.zeros(dim, dtype=np.float32)
    
    quarter = dim // 4
    
    if subtype == "SYN-Flood":
        # SYN Flood: massive SYN packets from many sources, no completed handshakes
        flow[:quarter] = rng.uniform(4.0, 8.0, quarter)                     # Many source IPs
        flow[quarter:quarter*2] = rng.uniform(-6.0, -4.0, quarter)          # No connection duration (no ACK)
        flow[quarter*2:quarter*3] = rng.uniform(-4.0, -2.0, quarter)        # Small SYN packets
        flow[quarter*3:] = rng.uniform(6.0, 10.0, dim - quarter*3)          # Extreme packet rate
    else:
        # UDP Flood: massive UDP datagrams, random ports, high bandwidth
        flow[:quarter] = rng.uniform(3.0, 7.0, quarter)                     # Many source IPs
        flow[quarter:quarter*2] = rng.uniform(-5.0, -3.0, quarter)          # No real connection
        flow[quarter*2:quarter*3] = rng.uniform(3.0, 6.0, quarter)          # Large UDP payloads
        flow[quarter*3:] = rng.uniform(7.0, 11.0, dim - quarter*3)          # Extreme bandwidth
    
    return flow.tolist()

def simulate_ddos(name, subtype, seed):
    dim = get_dimension()
    rng = np.random.RandomState(seed)
    
    print(f"\n  {B}{C}[DDoS Simulation] Threat: {name}{X}")
    print(f"  {'─'*50}")
    
    # 1. Learn the signature with structured features
    support_set = [generate_ddos_features(dim, rng, subtype) for _ in range(5)]
    ...
    query_flow = generate_ddos_features(dim, rng, subtype)
...
if __name__ == "__main__":
    print(f"\n{B}{'='*60}")
    print(f"  MetaShield — DDoS Attack Simulation")
    print(f"{'='*60}{X}")
    try:
    ...
    simulate_ddos("DDoS-SYN-Flood", "SYN-Flood", seed=200)
    simulate_ddos("DDoS-UDP-Flood", "UDP-Flood", seed=201)
