"""
MetaShield — Infiltration Test Script
======================================
Simulates Infiltration-Exploit and Infiltration-Rootkit attack subtypes.
...
def generate_infiltration_features(dim, rng, subtype="Exploit"):
    """
    Generate synthetic features that mimic Infiltration attack patterns.
    Infiltration traffic: standard entry port, tiny exploit request, large
    data exfiltration or reverse shell response.
    """
    flow = np.zeros(dim, dtype=np.float32)
    quarter = dim // 4
    
    if subtype == "Exploit":
        flow[:quarter] = rng.uniform(-0.5, 1.5, quarter)                    # Standard entry port
        flow[quarter:quarter*2] = rng.uniform(-1.0, 2.0, quarter)           # Normal connection duration
        flow[quarter*2:quarter*3] = rng.uniform(-3.0, -1.0, quarter)        # Tiny exploit payload
        flow[quarter*3:] = rng.uniform(5.0, 8.0, dim - quarter*3)           # Large reverse shell response
    else:
        flow[:quarter] = rng.uniform(3.0, 6.0, quarter)                     # Non-standard high ports (rootkit callback)
        flow[quarter:quarter*2] = rng.uniform(4.0, 7.0, quarter)            # Very long-lived connections
        flow[quarter*2:quarter*3] = rng.uniform(2.0, 5.0, quarter)          # Data exfiltration
        flow[quarter*3:] = rng.uniform(1.0, 3.0, dim - quarter*3)           # Encrypted traffic patterns
    
    return flow.tolist()

def simulate_infiltration(name, subtype, seed):
    dim = get_dimension()
    rng = np.random.RandomState(seed)
    
    print(f"\n  {B}{C}[Infiltration Simulation] Threat: {name}{X}")
    print(f"  {'─'*50}")
    
    # 1. Learn the signature
    support_set = [generate_infiltration_features(dim, rng, subtype) for _ in range(5)]
    ...
    query_flow = generate_infiltration_features(dim, rng, subtype)
...
if __name__ == "__main__":
    print(f"\n{B}{'='*60}")
    print(f"  MetaShield — Infiltration Attack Simulation")
    print(f"{'='*60}{X}")
    try:
    ...
    simulate_infiltration("Infiltration-Exploit", "Exploit", seed=700)
    simulate_infiltration("Infiltration-Rootkit", "Rootkit", seed=701)
    
    print(f"\n{B}{'='*60}")
    print(f"  ✅ Infiltration Simulation Complete")
    print(f"{'='*60}{X}\n")
